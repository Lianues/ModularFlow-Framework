# SmartTraven 前端开发文档：架构与项目结构

本文件定义 SmartTraven 前端应用（Vue + Vite + TypeScript + Tailwind CSS）的目标、目录结构、解耦策略、两种聊天显示模式（楼层对话 / 0 楼层 iframe 沙盒）、统一配置面板、代码高亮与 HTML 代码块渲染、安全与美化体系，以及构建与部署规范。遵循仓库统一开发规范（见根目录 DEVELOPMENT_NOTES.md）。
...
说明：以下结构是单仓库子项目 frontend_projects/SmartTraven 的 src 规划，方便二次开发与模块替换。文件名仅为建议，可按团队约定微调。

```
frontend_projects/SmartTraven/
├─ docs/
│  └─ 开发文档_架构与项目结构.md        # 本文档
├─ public/
│  ├─ favicon.ico
...
本文件作为 SmartTraven 前端的落地方案与约定基线。若遇到与后端或框架规范不一致的情况，请以仓库开发规范（DEVELOPMENT_NOTES.md）为准，并在本文件中同步更新。
---
...
实现文件建议：
- [frontend_projects/SmartTraven/src/features/themes/manager.ts](frontend_projects/SmartTraven/src/features/themes/manager.ts)
- [frontend_projects/SmartTraven/src/features/themes/pack.ts](frontend_projects/SmartTraven/src/features/themes/pack.ts)
- [frontend_projects/SmartTraven/src/features/themes/store.ts](frontend_projects/SmartTraven/src/features/themes/store.ts)
- [frontend_projects/SmartTraven/src/views/settings/ThemesView.vue](frontend_projects/SmartTraven/src/views/settings/ThemesView.vue)
...
沙盒宿主与桥接建议文件：
- [frontend_projects/SmartTraven/src/features/themes/sandbox/host.ts](frontend_projects/SmartTraven/src/features/themes/sandbox/host.ts)
- [frontend_projects/SmartTraven/src/features/themes/sandbox/client.ts](frontend_projects/SmartTraven/src/features/themes/sandbox/client.ts)
- [frontend_projects/SmartTraven/src/features/themes/sandbox/index.html](frontend_projects/SmartTraven/src/features/themes/sandbox/index.html)
...
- 应用启动顺序：
  1) 浏览器加载编译后的入口 [frontend_projects/SmartTraven/index.html](frontend_projects/SmartTraven/index.html)
  2) 前端入口 [frontend_projects/SmartTraven/src/app/main.ts](frontend_projects/SmartTraven/src/app/main.ts) 初始化 Pinia/Router
...
- 语义作用域（scope）标记（组件根节点）：
  - [data-scope="app-shell"]（AppShell 主布局，参考 [frontend_projects/SmartTraven/src/app/layouts/AppShell.vue](frontend_projects/SmartTraven/src/app/layouts/AppShell.vue)）
...
契约文档（随应用发布，便于主题作者查阅）：
- [frontend_projects/SmartTraven/public/themes/contract.json](frontend_projects/SmartTraven/public/themes/contract.json)
  - contractVersion：版本号
...
- Manifest 支持两种写法（向后兼容）：
  ...
  - 对象映射（推荐）：{"$app":"body[data-app='smarttraven']", "$appShell":"[data-scope='app-shell']", "$messageItem":"[data-scope='message-item']"}
...
- UI 导入：在“主题/主体（Themes）”页面直接导入 `.sttheme.json` 或 `.sttheme`，立即应用
  - 参考： [frontend_projects/SmartTraven/src/features/themes/manager.ts](frontend_projects/SmartTraven/src/features/themes/manager.ts)、[frontend_projects/SmartTraven/src/features/themes/pack.ts](frontend_projects/SmartTraven/src/features/themes/pack.ts)
...
- 主题作者文档（建议新增）：[frontend_projects/SmartTraven/docs/主题创作指南.md](frontend_projects/SmartTraven/docs/主题创作指南.md)
...
- 契约 JSON（随 dist 部署）：[frontend_projects/SmartTraven/public/themes/contract.json](frontend_projects/SmartTraven/public/themes/contract.json)
...
参考文档：
- 主题作者指南：[frontend_projects/SmartTraven/docs/主题创作指南.md](frontend_projects/SmartTraven/docs/主题创作指南.md)
...
- 稳定选择器（data- 标记）示例（仅说明）：
  - body[data-app="smarttraven"]、[data-scope="app-shell"]、[data-scope="message-item"]...
- 做法：
...
- 主题宿主与导入
  - 主题管理器（加载/解析/持久化/导出）：[frontend_projects/SmartTraven/src/features/themes/manager.ts](frontend_projects/SmartTraven/src/features/themes/manager.ts)
  - 主题包解析与校验： [frontend_projects/SmartTraven/src/features/themes/pack.ts](frontend_projects/SmartTraven/src/features/themes/pack.ts)
  - 运行时加载顺序：在应用挂载前应用 tokens + CSS，挂载后按开关可选启用脚本
- 沙盒桥接与 API
  - 宿主侧桥接/白名单 API： [frontend_projects/SmartTraven/src/features/themes/sandbox/host.ts](frontend_projects/SmartTraven/src/features/themes/sandbox/host.ts)
  - 客户端侧最小运行时： [frontend_projects/SmartTraven/src/features/themes/sandbox/client.ts](frontend_projects/SmartTraven/src/features/themes/sandbox/client.ts)
