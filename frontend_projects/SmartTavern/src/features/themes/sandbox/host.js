 ...
 * @param {string} [options.clientOrigin='*'] - Expected origin of client messages. Use specific origin in production.
 * @param {number} [options.timeoutMs=4000] - Request timeout for RPC-like calls.
 * @param {Object} [options.permissions] - Reserved permission gates, e.g. { dom:false, network:false }.
 */
export function createSandboxHost(iframe, options = {}) {
  ...
  function onMessage(ev) {
    ...
    const { id, type, payload, error } = data.__stSandbox
