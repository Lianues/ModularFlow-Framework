...
//
// 依赖：
// - ThemeManager（扩展注册/广播） [manager.js](../manager.js:1)
// - ThemeStore（令牌写入与持久化）   [store.js](../store.js:1)
//
// 行为：
 ...
 * @param {number|null|undefined} cardRadiusPx  卡片圆角 px（可能为 null 表示“默认”）
 * @param {number|null|undefined} msgOpacityPct 楼层消息背景不透明度（百分比 0~100）
 * @returns {string} CSS 阴影文本
 */
function deriveShadowMedium(cardRadiusPx, msgOpacityPct) {
 ...
 */
export async function registerDemoShadowFollowExtension() {
  const dispose = ThemeManager.registerExtension({
     ...
     * @param {import('@/types/theme').ThemeAppearanceSnapshot} snapshot
     */
