...
async function loadData() {
  ...
  try {
    const promises = [DataCatalog.listLLMConfigs()]
    ...
    // 处理列表
    llmConfigs.value = DataCatalog.mapToCards(listRes?.items || [], 'llm_configs')
    
    // 处理settings：标记使用中
    if (settingsRes?.settings?.llm_config) {
      usingKey.value = settingsRes.settings.llm_config
...
/**
 * 使用：调用settings API更新llm_config字段
 */
