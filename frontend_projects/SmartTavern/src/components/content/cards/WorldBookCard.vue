...
const form = ref({
  ...
  keysText: (props.entry.keys ?? []).join(';')
})
...
function onSave() {
  ...
  const keysArr =
    form.value.mode === 'conditional'
      ? String(form.value.keysText || '')
          .split(/;/)
          .map((x) => x.trim())
          .filter(Boolean)
      : []
...
<template>
  <div
    ...
    <div v-if="!editing" class="mt-3 space-y-2">
      ...
      <div v-if="props.entry.mode === 'conditional' && (props.entry.keys?.length || 0) > 0" class="text-xs text-black/60">
        keys：<span class="font-mono">{{ props.entry.keys.join(', ') }}</span>
      </div>
    </div>
    ...
    <div v-else class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div v-if="form.mode === 'conditional'" class="md:col-span-2">
        ...
        <input v-model="form.keysText" placeholder="示例：艾拉;工程师" class="w-full px-3 py-2 border border-gray-300 rounded-4 text-sm focus:outline-none focus:ring-2 focus:ring-gray-800" />
      </div>
