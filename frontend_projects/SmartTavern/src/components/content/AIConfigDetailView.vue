...
// 初始演示数据
const initialLLMConfigData = {
  name: '演示AI配置',
  description: 'OpenAI GPT-4 配置示例',
  provider: 'openai',
...
function normalizeLLMConfigData(src) {
  ...
  return {
    ...
    gemini_config: src.gemini_config || initialLLMConfigData.gemini_config,
    anthropic_config: src.anthropic_config || initialLLMConfigData.anthropic_config
  }
}
...
const currentData = ref(
  deepClone(
    normalizeLLMConfigData(props.llmConfigData) || initialLLMConfigData
  )
)

// 外部数据变更时同步
watch(() => props.llmConfigData, async (v) => {
  currentData.value = deepClone(normalizeLLMConfigData(v) || initialLLMConfigData)
  initJsonStrings()
  await nextTick()
...
async function save() {
  ...
  try {
    const res = await DataCatalog.updateLLMConfigFile(file, payloadContent, payloadContent.name, payloadContent.description)
    console.log('[AIConfigDetailView] 保存成功', res)
    savedOk.value = true
  } catch (e) {
    console.error('[AIConfigDetailView] 保存失败', e)
