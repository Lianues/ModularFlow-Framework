...
已创建 AI 对话补全工作流：`api/workflow/smarttraven/chat_completion/`

- **非流式**：`smarttraven/chat_completion/complete`
- **流式**：`smarttraven/chat_completion/complete_stream`
...
async function callAI() {
  try {
    const result = await ChatCompletion.complete({
      conversationFile: 'backend_projects/SmartTavern/data/conversations/branch_demo.json',
