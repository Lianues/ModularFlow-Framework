# SmartTraven 宏系统（传统宏 → Python）迁移说明与系统变量清单

说明：本迁移已将全部“传统宏”统一转换为受限 Python 片段，并交由沙盒 API 执行。调用方需要在 variables 中提供必要的“系统变量”，以替代原先依赖历史/输入缓存的宏取值。

相关实现文件
- [api/modules/SmartTraven/macro/impl.py](api/modules/SmartTraven/macro/impl.py)
- [api/modules/SmartTraven/macro/macro.py](api/modules/SmartTraven/macro/macro.py)
- [api/modules/SmartTraven/python_sandbox/impl.py](api/modules/SmartTraven/python_sandbox/impl.py)
- [api/modules/SmartTraven/python_sandbox/python_sandbox.py](api/modules/SmartTraven/python_sandbox/python_sandbox.py)

1) 执行链路（摘要）
- 宏解析与转换：在 macro/impl 中识别 {{...}} 或 <<...>>，传统宏会被转换为 Python 代码片段，再通过 SmartTraven.python_sandbox 执行，结果替换到 content 中，仅改写 content 字段，保留 source。
...
2) API 调用方式
- Endpoint: modules → smarttraven/macro/process
- 入参：
...
7) 与数据格式的对应关系（参考 backend_projects/SmartTraven/data）
- 人物与画像字段通常来自：
  - character: [backend_projects/SmartTraven/data/characters/*.json](backend_projects/SmartTraven/data/characters/心与露.json)
  - persona: [backend_projects/SmartTraven/data/persona/*.json](backend_projects/SmartTraven/data/persona/用户2.json)
- world_books 与预设说明见：
  - [backend_projects/SmartTraven/data/world_books/参考用main_world.json](backend_projects/SmartTraven/data/world_books/参考用main_world.json)
  - [backend_projects/SmartTraven/data/presets/Default.json](backend_projects/SmartTraven/data/presets/Default.json)
- 上游应在调用 smarttraven/macro/process 前，将上述字段抽取/映射为本文件第 3 节定义的系统变量键名。

8) 清理与兼容
- 旧模块 macro_module 与 python_sandbox_module 已废弃，调用请统一迁移至：
  - 宏处理：smarttraven/macro/process（POST）
  - Python 沙盒：smarttraven/python_sandbox/eval（POST）
...
- 相关文件：
  - [api/modules/SmartTraven/in_chat_constructor/impl.py](api/modules/SmartTraven/in_chat_constructor/impl.py)
  - [api/modules/SmartTraven/framing_prompt/impl.py](api/modules/SmartTraven/framing_prompt/impl.py)