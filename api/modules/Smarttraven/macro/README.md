# SmartTraven.macro 模块说明

位置：api/modules/SmartTraven/macro/

本模块提供“顺序宏处理”能力，支持在消息数组或单个纯文本中按从左到右、内层优先的策略解析并执行宏。宏仅替换 content（或纯文本）内容，不改变其他字段，并维护单一变量表（variables.initial → variables.final）。

相关代码
- 注册封装层：api/modules/SmartTraven/macro/macro.py
- 实现层：api/modules/SmartTraven/macro/impl.py
- 沙盒依赖：api/modules/SmartTraven/python_sandbox/python_sandbox.py
- 变量约定：api/modules/SmartTraven/macro/SYSTEM_VARIABLES.md

API 列表
- smarttraven/macro/process（处理消息数组）
- smarttraven/macro/process_text（处理纯文本）
...
API：messages 数组处理（smarttraven/macro/process）
- 输入
...
- 调用示例（Python SDK）
  - 示例代码：
    - core.call_api("smarttraven/macro/process", {"messages": [...], "variables": {...}}, method="POST", namespace="modules")

API：纯文本处理（smarttraven/macro/process_text）
- 输入
  - text: string
  - variables?: 对话上下文变量初值
- 输出
...
- 调用示例（Python SDK）
  - 示例代码：
    - core.call_api("smarttraven/macro/process_text", {"text": "a={{setvar:x:1}} b={{getvar:x}} c={{getvar:y}} d=<<python:3+4>>", "variables": {"pre": "v"}}, method="POST", namespace="modules")
  - 示例返回：
...
参考
- api/modules/SmartTraven/macro/macro.py
- api/modules/SmartTraven/macro/impl.py
- api/modules/SmartTraven/macro/SYSTEM_VARIABLES.md
- api/modules/SmartTraven/macro/test_macro.py