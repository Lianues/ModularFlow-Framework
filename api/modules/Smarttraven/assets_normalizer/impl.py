# -*- coding: utf-8 -*-
"""
SmartTraven.assets_normalizer 实现层
...
def _collect_rules_from_mixed(container: Any, add_source: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    # 1) 直接数组 / {"rules":[...]}
    if _is_list(container) or (_is_dict(container) and "rules" in container):
...
def extract_preset_regex_impl(preset: Dict[str, Any]) -> Dict[str, Any]:
    """
    提取预设中的 regex_rules，标准化为 {"rules":[...]} 并注入 meta.source="preset"
    """
    rules = []
    if _is_dict(preset):
...
def extract_character_regex_impl(character: Dict[str, Any]) -> Dict[str, Any]:
    """
    提取角色卡中的 regex_rules，标准化为 {"rules":[...]} 并注入 meta.source="character"
    """
    rules = []
...
def normalize_impl(
    ...
    rx_merge = merge_regex_impl(
        independent_regex={"rules": indep_rx},
    ...
    return {
        ...
        "merged_regex": rx_merge.get("merged_regex") or {"rules": []},
        "meta": {
            "stats": {
                ...
                "regex_final_count": len((rx_merge.get("merged_regex") or {}).get("rules") or []),
