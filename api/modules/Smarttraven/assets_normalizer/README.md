# 模块：SmartTraven.assets_normalizer

位置：`api/modules/SmartTraven/assets_normalizer/`

本模块提供“资产标准化”能力：针对 SmartTraven 项目的预设、世界书、角色卡与独立正则，进行必要的提取与合并，输出工作流友好的单体世界书与单体正则集合，解决下游模块无法直接处理“多世界书/多正则”的问题。
...
参考数据样例目录：`backend_projects/SmartTraven/data/`
...
模块命名空间：`/api/modules/smarttraven/assets_normalizer/*`

1) normalize（总入口）
- 路径：`smarttraven/assets_normalizer/normalize`
...
```json
{
  ...
  "character": { "…原样回传…" : true },
  "merged_regex": { "rules": [ { "…合并规则…" : true } ] },
  "meta": { "stats": { "…计数…" : 0 }, "order": { "regex": [], "world_book": [] }, "warnings": [] }
}
```

2) extract_preset_regex（提取预设正则）
- 路径：`smarttraven/assets_normalizer/extract_preset_regex`
- 输入：
```json
{ "preset": { "…完整预设…" : true } }
```
- 输出：
```json
{ "rules": [ { "…规则…" : true } ], "meta": { "source": "preset", "count": 1 } }
```

3) extract_character_world_book（提取角色卡世界书）
- 路径：`smarttraven/assets_normalizer/extract_character_world_book`
- 输入：
```json
{ "character": { "…完整角色卡…" : true } }
```
...
- 路径：`smarttraven/assets_normalizer/extract_character_regex`
- 输入：
```json
{ "character": { "…完整角色卡…" : true } }
```
- 输出：
```json
{ "rules": [ { "…规则…" : true } ], "meta": { "source": "character", "count": 5 } }
```

5) merge_world_books（世界书合并）
- 路径：`smarttraven/assets_normalizer/merge_world_books`
...
6) merge_regex（正则合并）
- 路径：`smarttraven/assets_normalizer/merge_regex`
- 说明：顺序为“独立正则 → 预设正则 → 角色卡正则”；内部采用默认最佳策略（去重键优先 id，其次 name，再次 find_regex；冲突保留先出现者），无需传入 `options`。
- 输入：
```json
{
  "independent_regex": [ { "…规则…" : true } ],
  "preset_regex": { "rules": [ { "…规则…" : true } ] },
  "character_regex": { "rules": [ { "…规则…" : true } ] }
}
```
- 输出：
```json
{
  "merged_regex": { "rules": [ { "…合并后唯一规则…" : true } ] },
  "meta": {
...
- 正则合并
  ...
  - 规则归一化：支持直接数组与 `{"rules":[…]}` 两种形态，并统一注入 `meta.source` 标识来源（independent/preset/character）。
...
res = core.call_api("smarttraven/assets_normalizer/normalize", payload, method="POST", namespace="modules")
print(res["world_book"][:1], res["merged_regex"]["rules"][:1])
```

- 提取与合并子功能（若仅需要部分能力）
```python
# 提取预设正则
core.call_api("smarttraven/assets_normalizer/extract_preset_regex", {"preset": preset_doc}, method="POST", namespace="modules")

# 提取角色卡世界书
core.call_api("smarttraven/assets_normalizer/extract_character_world_book", {"character": character_doc}, method="POST", namespace="modules")

# 提取角色卡正则
core.call_api("smarttraven/assets_normalizer/extract_character_regex", {"character": character_doc}, method="POST", namespace="modules")

# 合并世界书（仅 id 去重，不覆盖）
core.call_api("smarttraven/assets_normalizer/merge_world_books", {
...
core.call_api("smarttraven/assets_normalizer/merge_regex", {
...
```bash
python -u api/modules/SmartTraven/assets_normalizer/test_assets_normalizer.py
```
...
- 世界书输出为“条目数组”而非 `{"entries":[…]}` 包裹，确保与现有模块（例如 in_chat_constructor、framing_prompt）的输入惯例兼容。
