# -*- coding: utf-8 -*-
"""
API 封装层：SmartTraven.assets_normalizer
- 提供 6 个 API：
  1) normalize（总入口）
...
@core.register_api(
    path="smarttraven/assets_normalizer/normalize",
...
@core.register_api(
    path="smarttraven/assets_normalizer/extract_preset_regex",
    name="提取预设正则",
    description="从预设完整JSON中提取 regex_rules 字段并标准化为 {'rules':[...]}，为每条规则注入 meta.source='preset'。",
    input_schema={
        "type": "object",
        "properties": {
            "preset": {"type": "object", "additionalProperties": True}
...
@core.register_api(
    path="smarttraven/assets_normalizer/extract_character_world_book",
    name="提取角色卡世界书",
...
@core.register_api(
    path="smarttraven/assets_normalizer/extract_character_regex",
    name="提取角色卡正则",
    description="从角色卡完整JSON中提取 regex_rules，标准化为 {'rules':[...]}，为每条规则注入 meta.source='character'。",
    input_schema={
        "type": "object",
        "properties": {
            "character": {"type": "object", "additionalProperties": True}
...
@core.register_api(
    path="smarttraven/assets_normalizer/merge_world_books",
    name="世界书合并",
...
@core.register_api(
    path="smarttraven/assets_normalizer/merge_regex",
    name="正则合并（独立→预设→角色卡）",
