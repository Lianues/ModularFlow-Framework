...
# -*- coding: utf-8 -*-
"""
测试：SmartTavern.assets_normalizer 模块（6个API）
- 允许使用仓库内样例数据进行集成测试
"""
...
def test_extract_preset_regex(preset_doc):
    res = core.call_api(
        "smarttavern/assets_normalizer/extract_preset_regex",
        {"preset": preset_doc},
        method="POST",
        namespace="modules",
    )
...
def test_extract_character_world_book(character_doc):
    res = core.call_api(
        "smarttavern/assets_normalizer/extract_character_world_book",
...
def test_extract_character_regex(character_doc):
    res = core.call_api(
        "smarttavern/assets_normalizer/extract_character_regex",
        {"character": character_doc},
        method="POST",
        namespace="modules",
    )
...
def test_merge_world_books(world_books_doc, char_wb_items):
    ...
    res = core.call_api(
        "smarttavern/assets_normalizer/merge_world_books",
        payload,
        method="POST",
        namespace="modules",
    )
...
def test_merge_regex(independent_rules_doc, preset_rules, char_rules):
    ...
    res = core.call_api(
        "smarttavern/assets_normalizer/merge_regex",
        payload,
        method="POST",
        namespace="modules",
    )
...
def main():
    ...
    # 加载样例数据
    preset_doc = _read_json("backend_projects/SmartTavern/data/presets/Default.json")
    world_books_doc = _read_json("backend_projects/SmartTavern/data/world_books/参考用main_world.json")
    character_doc = _read_json("backend_projects/SmartTavern/data/characters/心与露.json")
    independent_rules_doc = _read_json("backend_projects/SmartTavern/data/regex_rules/remove_xml_tags.json")
