...
- 实现层（核心逻辑）：[python.in_chat_constructor.impl.py](api/modules/SmartTavern/in_chat_constructor/impl.py:1)
  ...
  - 历史文本拼接（用于 keys 命中判定）：[python._collect_history_text()](api/modules/SmartTavern/in_chat_constructor/impl.py:67)
  - 条件触发（conditional + keys）：[python._is_triggered_by_keys()](api/modules/SmartTavern/in_chat_constructor/impl.py:79)
...
- 条件触发 keys（区分大小写）：
  - mode=="conditional" 且 keys 命中历史文本（由 [python._collect_history_text()](api/modules/SmartTavern/in_chat_constructor/impl.py:67) 拼接）时才注入
...
- 世界书候选：
  - mode=="always"：直接加入候选
  - mode=="conditional"：仅当 keys 命中历史拼接文本时加入（见 [python._is_triggered_by_keys()](api/modules/SmartTavern/in_chat_constructor/impl.py:79)）
