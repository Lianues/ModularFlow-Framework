...
def retry_branch_impl(
    ...
    """
    重试分支：创建与指定节点同级的新分支节点，并切换 active_path 到新节点。
    
    参数：
    - old_node_id: 要重试的节点 ID（必须存在）
    ...
    返回：
      更新后的完整 doc（nodes 新增，children 添加新节点，active_path 替换最后节点，updated_at 更新）
    
    逻辑：
    1. 继承 old_node_id 的 pid
    2. 创建新节点，添加到 pid 的 children 末尾
    3. 将 active_path 中的 old_node_id 替换为 new_node_id
    """
    loaded_doc = _load_doc_from_file_or_obj(doc, file)
    nodes = loaded_doc.get("nodes") or {}
    children_map = loaded_doc.get("children") or {}
    ...
    if old_node_id not in nodes:
        raise ValueError(f"Old node not found: {old_node_id}")
    ...
    # 更新父节点 children（添加到末尾）
    if pid not in children_map:
        children_map[pid] = []
    if new_node_id not in children_map[pid]:
        children_map[pid].append(new_node_id)
    
    # 替换 active_path 中的 old_node_id 为 new_node_id
    if old_node_id in active_path:
        idx = active_path.index(old_node_id)
        active_path[idx] = new_node_id
        # 截断后续路径（重试后的路径只到新节点）
